...
if(isset($_SESSION['LoggedInUser']) && $_SESSION['permission'] == 1)
{
    ...
    if(isset($_POST['comOrder']))
    {
        $query = "UPDATE orders SET order_status = 1 WHERE id = ".$_POST['comOrder']."";
        $result = mysqli_query($link, $query) or die("Error: an error has occurred while executing the query");

    }
    $query = "SELECT * FROM orders WHERE is_cart = 1";
    ...
    {
        $customer = "SELECT * FROM users WHERE id = ".$row['customer_id']."";
        ...
        echo'</div>
            ...
            {
                echo'
            <button id="completed_order"   onclick="comOrder('.$row['id'].');">Confirm</button>';
            }
